...
function title() {
	...
	console.log(color(`\n${themeemoji} YT CHANNEL: UNKNOWN`,'magenta'))
console.log(color(`${themeemoji} GITHUB: UNKNOWN `,'magenta'))
console.log(color(`${themeemoji} WA NUMBER: ${owner}`,'magenta'))
console.log(color(`${themeemoji} CREDIT: ${wm}\n`,'magenta'))
}

async function XeonBotIncBot() {
    	const { state, saveCreds } = await useMultiFileAuthState('Fanz')
        const XeonBotInc = XeonBotIncConnect({
            printQRInTerminal: true,
            logger: pino({ level: 'fatal' }),
            auth: state,
            browser: [`${botname}`, "FanzBotzz", "3.0"],
...
console.log(color(figlet.textSync(`FanzBotzz`, {
font: 'Standard',
